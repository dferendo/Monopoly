...
// TODO throw exception if out of range??
    ...
    std::ostream &operator<<(std::ostream &stream, const HousesPrice &housesPrice) {
        return stream << "Price of house starting from 1: " << housesPrice.oneHouse << ", " << housesPrice.secondHouse
                      << ", " << housesPrice.thirdHouse << ", " << housesPrice.fourHouse << ". Price to upgrade: "
                      << housesPrice.priceToUpgrade;
    }
}